...
public class ConcreteSyntax {
	...
	private String SyntaxError(String tokenVal) {
		String errorMessage = "Syntax error - Expecting: " + tokenVal + " But saw: " + token.getType() + " = " + token.getValue();
	...
	private Expression conjunction() {
		...
		while (token.getValue().equals("&&")) {
			binary = new Binary();
			binary.term1 = exp;
			binary.op = new Operator(token.getValue());
			token = input.nextToken();
			binary.term2 = relation();
			exp = binary;
		}
		return exp;
	}
